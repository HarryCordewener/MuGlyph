...
- **Snapshot view names:** `worlds`/`settings`, `triggers`, `route`, `highlight`,
  ...
  `drag`, `history`, `draft`, `draft2`, `menu`, `menu-split`, plus the default (no `--view`)
  workspace. (`input` is the **F8 settings screen**; `draft`/`draft2` are the **command line**
  itself — a wrapped draft that has grown the bar, and the same with the per-window second bar
  raised and ⏎ armed on it.)
