...
SharpMUTerm can render its UI to an image **without a terminal or a live connection**, so
...
VHS drives the published `sharpmuterm` binary in a headless terminal and records the result —
