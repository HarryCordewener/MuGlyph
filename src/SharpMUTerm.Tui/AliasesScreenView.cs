...
internal static class AliasesScreenView
{
    ...
    {
        var header = Band(AliasesScreenRenderer.HeaderLine(width), AliasesScreenRenderer.HeaderBg);
        var footer = Band(AliasesScreenRenderer.FooterLine(sets, selected, width), AliasesScreenRenderer.FooterBg);

        // Body: alias list │ editor, as two real columns.
        var listCol = Stretch(new MarkupControl(AliasesScreenRenderer.ListColumn(sets, selected)));
        var editorCol = Stretch(new MarkupControl(Indent(AliasesScreenRenderer.EditorColumn(sets, selected))));
