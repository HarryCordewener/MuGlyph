...
internal sealed class SharpMUTermApp : IAsyncDisposable
{
    ...
    /// <summary>
    /// Replaces the status line, and re-derives the input-height veto against it. The veto counts the
    /// rows the header and the status line wrap to, and the status line is the one piece of chrome whose
    /// length changes at runtime — <c>not connected</c> at construction, a full identity + keepalive +
    /// host cluster once a session is up, which on a narrow terminal is a second row the bars were
    /// promised. Left uncounted, an 80×6 window handed the input area a row the status line then took.
    /// </summary>
